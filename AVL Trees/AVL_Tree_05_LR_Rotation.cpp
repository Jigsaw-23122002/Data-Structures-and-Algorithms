    ...
    int AVL(node *root,node *head,bool &status){
        if(head==NULL){
            ...
            if(heightLeft-heightRight>1){
                cout<<"\n Initialising the RL rotation on the node with value "<<head->data;
...
int main(){
    ...
    cout<<"\n Enter the number of nodes present in the Binary Search Tree(Requiring Only Left-Right Rotation to be AVL):";
    cin>>n;
    vector<string> initialisation(n);
    cout<<"\n Enter the node values of the Binary Search Tree(Requiring Only Left-Right Rotation to be AVL):";
    for(int i=0;i<n;i++){
        cin>>initialisation[i];
    }
    AVLTree avl(initialisation);
    cout<<"\n The Status of the Binary Search Tree before the Left-Right Rotation:";
    ...
    status=true;
    avl.AVL(avl.head,avl.head,status);
    cout<<"\n The Status of the Binary Search Tree After Left-Right Rotation:";
